...
public class InductionMotor {
	 ...
	 * @param v1 tensão de fase de armadura, em Volts
	 * @param r1 resistência dos enrolamento da armadura, em Ohms
	 * @param r2 resistência dos enrolamentos do rotor, em Ohms
	 * @param x1 reatância de dispersão dos enrolamentos da armadura, em Ohms
	 * @param x2 reatância de dispersão dos enrolamentos do rotor, em Ohms
	 * @param xm reatância de magnetização, em Ohms
	 ...
	 * @return matriz com duas linhas e um número de colunas igual ao número de
	 *         elementos do vetor de escorregamento, contendo a velocidade mecânica,
	 *         em Hz, e o torque, em N.m
	 */
	public static double[][] tm(int nf, double v1, double r1, double r2, double x1, double x2, double xm, double fo,
		...
		for (int i = 0; i < s.length; i++) {
			...
			double v1eq2 = v1eq.getMod2();

			double r1eq = z1eq.getRe();
			double x1eq = z1eq.getIm();

			if (s[i] != 0.)
				ts[i] = nf * v1eq2 * (1 / (2 * Math.PI * ns)) * (r2 / s[i])
